...
static void CAN_Mode_Config(void)
{
  ...
  RCC_APB1PeriphClockCmd(CAN_CLK, ENABLE);
	...
	CAN_InitStructure.CAN_Prescaler =16;		   ////BTR-BRP 波特率分频器  定义了时间单元的时间长度 36/(1+5+3)/4=1 Mbps
...
//	CAN_FilterInitStructure.CAN_FilterIdLow= (((u32)0x1314<<3)|CAN_ID_EXT|CAN_RTR_DATA)&0xFFFF; //要筛选的ID低位 
	CAN_FilterInitStructure.CAN_FilterIdHigh= (((u32)slave_id<<21)&0xffff0000)>>16;		//要筛选的ID高位 
	CAN_FilterInitStructure.CAN_FilterIdLow= (((u32)slave_id<<21)|CAN_ID_STD|CAN_RTR_DATA)&0xFFFF; //要筛选的ID低位 
 ...
 * 调用  ：外部调用
 */	 
void CAN_SetMsg(CanTxMsg *TxMessage , uint8_t *TxData)
{	  
	uint8_t ubCounter = 0;

  TxMessage->StdId=0x121;						                 //1314--121 ysm
  //TxMessage->ExtId=0x1314;					 //使用的扩展ID
  TxMessage->IDE=CAN_ID_STD;					 //扩展模式       //EXT--STD ysm
  TxMessage->RTR=CAN_RTR_DATA;				 //发送的是数据
  TxMessage->DLC=2;							 //数据长度为8字节       8--2 ysm
	
	/*设置要发送的数据0-7*/
	for (ubCounter = 0; ubCounter < 2; ubCounter++)         //8--2 ysm
  {
